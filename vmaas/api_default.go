...
type ApiVmaasWebappAppPkgListHandlerPostPostRequest struct {
	ctx _context.Context
	ApiService *DefaultApiService
	pkgListRequest *PkgListRequest
}

func (r ApiVmaasWebappAppPkgListHandlerPostPostRequest) PkgListRequest(pkgListRequest PkgListRequest) ApiVmaasWebappAppPkgListHandlerPostPostRequest {
	r.pkgListRequest = &pkgListRequest
	return r
}

func (r ApiVmaasWebappAppPkgListHandlerPostPostRequest) Execute() (PkgListResponse, *_nethttp.Response, error) {
	return r.ApiService.VmaasWebappAppPkgListHandlerPostPostExecute(r)
}

/*
 * VmaasWebappAppPkgListHandlerPostPost Method for VmaasWebappAppPkgListHandlerPostPost
 * Get details about all packages..
 * @param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @return ApiVmaasWebappAppPkgListHandlerPostPostRequest
 */
func (a *DefaultApiService) VmaasWebappAppPkgListHandlerPostPost(ctx _context.Context) ApiVmaasWebappAppPkgListHandlerPostPostRequest {
	return ApiVmaasWebappAppPkgListHandlerPostPostRequest{
		ApiService: a,
		ctx: ctx,
	}
...
/*
 * Execute executes the request
 * @return PkgListResponse
 */
func (a *DefaultApiService) VmaasWebappAppPkgListHandlerPostPostExecute(r ApiVmaasWebappAppPkgListHandlerPostPostRequest) (PkgListResponse, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod   = _nethttp.MethodPost
		localVarPostBody     interface{}
		localVarFormFileName string
		localVarFileName     string
		localVarFileBytes    []byte
		localVarReturnValue  PkgListResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "DefaultApiService.VmaasWebappAppPkgListHandlerPostPost")
	if err != nil {
		return localVarReturnValue, nil, GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/pkglist"
	...
	// body params
	localVarPostBody = r.pkgListRequest
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, localVarFormFileName, localVarFileName, localVarFileBytes)
	if err != nil {
		return localVarReturnValue, nil, err
	}

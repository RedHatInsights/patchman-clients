/*
 * Insights Host Inventory REST Interface
 *
 * REST interface for the Insights Platform Host Inventory application.
 *
 * API version: 1.0.0
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package inventory

import (
	"encoding/json"
)

// PaginationOut struct for PaginationOut
type PaginationOut struct {
	// The number of items on the current page
	Count int32 `json:"count"`
	// The page number
	Page int32 `json:"page"`
	// The number of items to return per page
	PerPage int32 `json:"per_page"`
	// Total number of items
	Total int32 `json:"total"`
}

// NewPaginationOut instantiates a new PaginationOut object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPaginationOut(count int32, page int32, perPage int32, total int32, ) *PaginationOut {
	this := PaginationOut{}
	this.Count = count
	this.Page = page
	this.PerPage = perPage
	this.Total = total
	return &this
}

// NewPaginationOutWithDefaults instantiates a new PaginationOut object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPaginationOutWithDefaults() *PaginationOut {
	this := PaginationOut{}
	return &this
}

// GetCount returns the Count field value
func (o *PaginationOut) GetCount() int32 {
	if o == nil  {
		var ret int32
		return ret
	}

	return o.Count
}

// GetCountOk returns a tuple with the Count field value
// and a boolean to check if the value has been set.
func (o *PaginationOut) GetCountOk() (*int32, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Count, true
}

// SetCount sets field value
func (o *PaginationOut) SetCount(v int32) {
	o.Count = v
}

// GetPage returns the Page field value
func (o *PaginationOut) GetPage() int32 {
	if o == nil  {
		var ret int32
		return ret
	}

	return o.Page
}

// GetPageOk returns a tuple with the Page field value
// and a boolean to check if the value has been set.
func (o *PaginationOut) GetPageOk() (*int32, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Page, true
}

// SetPage sets field value
func (o *PaginationOut) SetPage(v int32) {
	o.Page = v
}

// GetPerPage returns the PerPage field value
func (o *PaginationOut) GetPerPage() int32 {
	if o == nil  {
		var ret int32
		return ret
	}

	return o.PerPage
}

// GetPerPageOk returns a tuple with the PerPage field value
// and a boolean to check if the value has been set.
func (o *PaginationOut) GetPerPageOk() (*int32, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.PerPage, true
}

// SetPerPage sets field value
func (o *PaginationOut) SetPerPage(v int32) {
	o.PerPage = v
}

// GetTotal returns the Total field value
func (o *PaginationOut) GetTotal() int32 {
	if o == nil  {
		var ret int32
		return ret
	}

	return o.Total
}

// GetTotalOk returns a tuple with the Total field value
// and a boolean to check if the value has been set.
func (o *PaginationOut) GetTotalOk() (*int32, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Total, true
}

// SetTotal sets field value
func (o *PaginationOut) SetTotal(v int32) {
	o.Total = v
}

func (o PaginationOut) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["count"] = o.Count
	}
	if true {
		toSerialize["page"] = o.Page
	}
	if true {
		toSerialize["per_page"] = o.PerPage
	}
	if true {
		toSerialize["total"] = o.Total
	}
	return json.Marshal(toSerialize)
}

type NullablePaginationOut struct {
	value *PaginationOut
	isSet bool
}

func (v NullablePaginationOut) Get() *PaginationOut {
	return v.value
}

func (v *NullablePaginationOut) Set(val *PaginationOut) {
	v.value = val
	v.isSet = true
}

func (v NullablePaginationOut) IsSet() bool {
	return v.isSet
}

func (v *NullablePaginationOut) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePaginationOut(val *PaginationOut) *NullablePaginationOut {
	return &NullablePaginationOut{value: val, isSet: true}
}

func (v NullablePaginationOut) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePaginationOut) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


